...
def inference(images):
  ...
  def binFullyConnected(reshape, weights, phase=True):
    ...
    activ = tf.sign(reshape)
    local = tf.matmul(activ, tf.sign(weights))
    return local
  ...
  norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                    name='norm1')
  """ End of original conv layer 1 """
                  
  """ Testing... binarized conv layer 1 """
  # # conv1
  # with tf.variable_scope('bconv1') as scope:
  #   kernel = _variable_with_weight_decay('weights',
  #                                        shape=[5, 5, 3, 64],
  #                                        stddev=5e-2,
  #                                        wd=0.0)
  #   conv1 = binConvolution(images, kernel, [1, 1, 1, 1], padding='SAME')
  #   _activation_summary(conv1)
  ...
  # # Just to make it easy to switch between binarized and non-binarized layers
  # norm1 = pool1
  ...
  """ End of original linear layer """

  """ Testing... binarized linear layer """ 
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [192, NUM_CLASSES],
                                          stddev=1/192.0, wd=0.0)
    biases = _variable_on_cpu('biases', [NUM_CLASSES],
                              tf.constant_initializer(0.0))
    softmax_linear = binFullyConnected(local4, weights) # training
    # softmax_linear = binFullyConnected(local4, weights, False) # testing
    _activation_summary(softmax_linear)
  """ End of binarized linear layer """
