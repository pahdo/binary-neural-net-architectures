...
def inference(images):
  ...
  """ fp16 conv layer 9 - 1x1 convolution is a projection """
  ...
  with tf.variable_scope('avgpool') as scope:
    pool = tf.nn.avg_pool(conv9, ksize=[1, 6, 6, 1], strides=[1, 1, 1, 1], padding='SAME', name='pool')
    pool = tf.reshape(pool, [FLAGS.batch_size, -1])

  """ Original linear layer """ 
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [360, NUM_CLASSES],
                                          stddev=1/360.0, wd=0.0)
    biases = _variable_on_cpu('biases', [NUM_CLASSES],
                              tf.constant_initializer(0.0))
    softmax_linear = tf.add(tf.matmul(pool, weights), biases, name=scope.name)
